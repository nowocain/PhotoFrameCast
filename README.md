PhotoFrameCast 🖼️
A Home Assistant custom integration for casting photo slideshows to media players, perfect for digital photo frames.
...
Open the HACS panel in your Home Assistant instance.

Go to Integrations.

Click on the three dots in the top right corner and select Custom repositories.

Enter the URL for this repository https://github.com/your-github-username/photoframecast and select the category Integration.

Click ADD.

You should now see "PhotoFrameCast" in the HACS Integrations list. Click on it and then click DOWNLOAD.

Add the following line to your configuration.yaml file to expose the services:
...
Manual Installation
Navigate to your Home Assistant custom_components directory. If it doesn't exist, create it.

Create a new folder named photoframecast inside custom_components.

Copy all the files from the custom_components/photoframecast folder in this repository into your new folder.

Add the following line to your Home Assistant configuration.yaml file to expose the services:
...
alias: Start Slideshow on Living Room TV
trigger:
  - platform: state
    entity_id: media_player.living_room_tv
    to: 'on'
action:
  - service: photoframecast.start_slideshow
    data:
      entity_id: media_player.living_room_tv
      folder: /media/vacation_photos
      interval: 10
      loop: false
      shuffle: true
photoframecast.stop_slideshow
Stops any active slideshow on a specified media player. This is the recommended way to programmatically stop a running slideshow to ensure resources are properly cleaned up.
...
How Slideshow Options Work Together
...
shuffle	resume	sort_folder_by_folder	Photo Display Order
true	true or false	true or false	The slideshow will always start from a new, randomized list of all photos. The resume and sort_folder_by_folder options are ignored because shuffling creates a new order each time.
false	true	true	The slideshow resumes from where it was last stopped. Photos are displayed in alphabetical order, sorted by folder. The order is consistent between sessions.
false	true	false	The slideshow resumes from where it was last stopped. The entire library of photos is treated as a single list and sorted alphabetically, regardless of folders. The order is consistent between sessions.
false	false	true	The slideshow starts from the beginning of the photo list. Photos are displayed in alphabetical order, sorted by folder.
false	false	false	The slideshow starts from the beginning of the photo list. The entire library is sorted alphabetically as one single list.


Troubleshooting
Slideshow delay: When a photo folder contains a very large number of files, the initial loading of the photo list can take a few seconds or even minutes. The slideshow will begin automatically once the list is fully prepared.

Large Photo Library: If your Home Assistant instance has limited storage, you can mount a Samba network share directly to your Home Assistant media folder. This allows you to store your photos externally while still having them accessible via the /media path.

No photos found: Double-check that the folder path is correct and that your media_dirs configuration is set up properly. Also, ensure Home Assistant has permissions to read the specified folder and that the photo files are in a supported format (.jpg, .jpeg, .png).

"Failed to cast URL..." or "slideshow aborted" or ": Ensure your media player supports the play_media service for images.

Slideshow stops unexpectedly: This could be due to a network interruption or the media player itself ending the session. Check your max_runtime setting to ensure it is not causing the slideshow to end prematurely.
